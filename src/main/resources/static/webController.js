...
function loadTopics() {
  ...
  fetch(`http://localhost:7070/search/topics?kafkaAddress=${encodeURIComponent(kafkaAddress)}`)
    .then(res => res.json())
    .then(data => {
      allTopics = data.sort((a, b) => a.localeCompare(b)); // sadece autocomplete için dizi
  ...
  function searchKafka() {
    ...
    if (mode === "manual") {
      if (isNaN(startOffset) || isNaN(endOffset) || startOffset < 0 || endOffset < startOffset) {
        return alert("Lütfen geçerli bir offset aralığı giriniz.");
      }
    }
...
box.appendChild(summary);
    ...
    .catch(err => {
      ...
      alert("Kafka sorgusu sırasında hata oluştu.");
    });
  }
